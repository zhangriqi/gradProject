...
class W2VEmbReader:
    ...
    def __init__(self, emb_path, emb_dim=None):
        ...
        print('emb_matrix: ', np.shape(emb_matrix))
    ...
    def get_emb_matrix_given_vocab(self, vocab, emb_matrix):
        counter = 0.
        # print('self.embeddings: ', np.shape(self.embeddings), type(self.embeddings))
        # print(vocab)
        # for word, index in vocab.items():
            # print(word, index)
            # print('emb_matrix: ', emb_matrix[index])
            # # print('embeddings: ', self.embeddings[word])
            # try:
            #     emb_matrix[index] = self.embeddings[word] #self.embeddings is a dict
            #     counter += 1
            # except KeyError:
            #     pass
